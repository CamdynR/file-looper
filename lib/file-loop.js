    ...
    for (let i = 0; i < numRuns; i++) {
      ...
      filePaths.forEach(filePath => {
        ...
        this.plugins.forEach(plugin => {
          ...
          // Check plugin onlyDo requirements
          if (plugin.options?.onlyDo) {
            // Check if a filName is specified
            if (plugin.options?.fullFilePaths) {
              if (!plugin.options.fullFilePaths.includes(file.fullFilePath)) {
                return;
              }
            }
            // Check if a fullFileName is specified
            if (plugin.options?.fullFileNames) {
              if (!plugin.options.fullFileNames.includes(file.fullFileName)) {
                return;
              }
            }
            // Check if a fileName is specified
            if (plugin.options?.fileNames) {
              if (!plugin.options.fileNames.includes(file.fileNames)) {
                return;
              }
            }
            // Check if a directory is specified
            if (plugin.options?.directories) {
              let fileDirectory = file.fullFilePath.split(this.delimiter);
              fileDirectory.pop();
              fileDirectory = fileDirectory.join(this.delimeter);
              for (let i = 0; i < plugin.options.directories.length; i++) {
                if (!plugin.options.directories[i].endsWith(fileDirectory)) {
                  return;
                }
              }
            }
            // Check if an extension is specified
            if (plugin.options?.extensions) {
              if (!plugin.options.extensions.includes(file.extension)) {
                return;
              }
            }
          }

          // Check plugin exclude requirements
          if (plugin.options?.exclude) {
            // TODO
          }

          // Check .run() onlyDo requirements
          if (options?.exclude) {
            // TODO
          }

          // Check .run() exclude requirements
          if (options?.exclude) {
            // TODO
          }
